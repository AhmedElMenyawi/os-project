...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	va = (uint32*)kmalloc(PAGE_SIZE);
	pa = kheap_physical_address((unsigned int)va);
	//link it to the given directory and return the address of the created table
	pa = ROUNDDOWN(pa,PAGE_SIZE)/PAGE_SIZE;
	uint32 offset = pa << 12;
	ptr_page_directory[PDX(virtual_address)] = offset | PERM_PRESENT | PERM_USER | PERM_WRITEABLE;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(uint32 i = VA; i< (VA+size); i+=PAGE_SIZE)
	{
		...
		env_page_ws_invalidate(e,i); //remove ws pages
		//e->page_last_WS_index = i; //update ws
		unmap_frame(e->env_page_directory, (void*)i);
		uint32 * ptr_page_table = NULL;
		struct Frame_Info* ptr = get_frame_info(e->env_page_directory, (void*)i, &ptr_page_table);
