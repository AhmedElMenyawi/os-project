...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	uint32 workingSetPages = env_page_ws_get_size(curenv);
	struct Frame_Info *ptr = NULL;
	//if the size of the page working set < its max size,
	// PLACEMENT
	if (workingSetPages < curenv->page_WS_max_size)
	{
		//1. allocate and map a frame for the faulted page
		int fr = allocate_frame(&ptr);
		if (fr != E_NO_MEM)
		{
			fr = map_frame(curenv->env_page_directory , ptr, (void*) fault_va, PERM_USER | PERM_WRITEABLE | PERM_PRESENT);
		}
		//2. read the faulted page from page file to memory
		int faultedPage = pf_read_env_page(curenv, (void*)fault_va);
		//3. If the page does not exist on page file,
		if (faultedPage == E_PAGE_NOT_EXIST_IN_PF)
		{
	...
	{
		//5. implement the FIFO algorithm to find victim virtual address from page working set to replace
		//if the last added page is the max size, then remove the first page
		//6. for the victim page: if modified update its page in page file,then unmap it
		uint32 VA = env_page_ws_get_virtual_address(curenv, curenv->page_last_WS_index);
		uint32 page_permisions = pt_get_page_permissions(curenv, VA);
		uint32* table_ptr = NULL;
		struct Frame_Info*ptr_frame_info = get_frame_info(curenv->env_page_directory, (uint32*)VA, &table_ptr);
		//if modified
		if (page_permisions & PERM_MODIFIED)
		{
			pf_update_env_page(curenv, (uint32*)VA, ptr_frame_info);
			unmap_frame(curenv->env_page_directory, (void*)VA);
		}
		//6. for the victim page:If not modified,then:unmap it
		else
		{
			unmap_frame(curenv->env_page_directory, (void*)VA);
		}
		//7. Reflect the changes in the page working set
		//env_page_ws_set_entry(curenv, curenv->page_last_WS_index , fault_va);
		//curenv->page_last_WS_index++;
		//8. Apply Placement steps.
		// PLACEMENT
		///1. allocate and map a frame for the faulted page
		int fr = allocate_frame(&ptr);
		if (fr != E_NO_MEM)
