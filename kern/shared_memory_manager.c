...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	int obj = allocate_share_object(&allocatedObject);
	if(obj == E_NO_SHARE)
		return E_NO_SHARE;
	else
	{
		...
		{
			allocate_frame(&pfi);
			map_frame(myenv->env_page_directory,pfi,virtual_address,PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
			virtual_address+=PAGE_SIZE;
			add_frame_to_storage(shares[obj].framesStorage,pfi,j);
			j++;
		}
...
int getSizeOfSharedObject(int32 ownerID, char* shareName)
{
	...
	int obj = get_share_object_ID(ownerID,shareName);
	if(obj == E_SHARED_MEM_NOT_EXISTS)
		return E_SHARED_MEM_NOT_EXISTS;
	else
		return shares[obj].size;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	else
	{
		struct Frame_Info *pfi;
		int j=0;
		int size = ROUNDUP(shares[obj].size, PAGE_SIZE);
		for(int i=0;i<size;i+=PAGE_SIZE)
		{
			pfi = get_frame_from_storage(shares[obj].framesStorage,j);
			if(shares[obj].isWritable == 1){
				map_frame(myenv->env_page_directory,pfi,virtual_address,PERM_USER|PERM_WRITEABLE| PERM_PRESENT);}
			else{
				map_frame(myenv->env_page_directory,pfi,virtual_address,PERM_USER| PERM_PRESENT);
			}
			j++;
			virtual_address+=PAGE_SIZE;
		}
